//! Run with:
//! cargo run --example bimodal_ke_dsl_runtime_native_aot --features "dsl-aot dsl-aot-load"
...
"#;
    ...
    // 1. Compile the model to the runtime Native AoT backend.
    let workspace = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("target")
        .join("example-artifacts")
        .join("bimodal_ke_dsl_runtime_native_aot");
    ...
    println!("bimodal_ke compiled with runtime Native AoT");
...
fn main() {
    eprintln!(
        "Run with: cargo run --example bimodal_ke_dsl_runtime_native_aot --features \"dsl-aot dsl-aot-load\""
    );
    std::process::exit(1);
}
