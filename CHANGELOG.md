...
- `Prediction::outeq()` returns `&OutputLabel` instead of `usize`. A prediction now carries the output's real label all the way from the data, so a model with `outputs: [cp, effect]` reports `"cp"` rather than `0` and callers no longer have to know the declared ordering. The dense slot is kept internally for hot-path indexing but is no longer public. `Observation::to_prediction` is removed; predictions are produced by the simulator, which is the only place that knows the slot.
- NCA output selection is by label. `NCAOptions::outeq` is `Option<OutputLabel>` instead of `usize` and `NCAOptions::with_outeq` takes a label (`with_outeq("cp")`). `None` — the default — selects the first output present in the data, which is what single-output callers want. The `ObservationMetrics` methods (`auc`, `auc_blq`, `auc_interval*`, `cmax*`, `tmax*`, `clast*`, `tlast*`, `aumc*`, and the `_first` variants) and `nca::sparse::sparse_auc_from_data` all take an output label instead of an index. `MetricsError::OutputEquationNotFound` is removed; a missing label is reported by `ObservationError::NoObservations`, which now carries the requested label together with the labels that *are* present.
- Numeric data labels are no longer aliased to `outeq_<n>` / `input_<n>` declarations. A model that declares named outputs and routes is matched only on the exact declared name, so a Pmetrics file with `OUTEQ = 1` no longer resolves against an output declared as `outeq_1`, and `INPUT = 1` no longer resolves against a route declared as `input_1`. Migration: rename the `OUTEQ` / `INPUT` column values to the declared label (`outeq_1`, `input_1`, `cp`, `iv`, …). The runtime error names the declared labels and says exactly this. Models built *without* metadata are unaffected: there are no declared names to match against, so a numeric label still resolves positionally to a dense slot.
- `.add(<number>, ..)` on `AssayErrorModels` is an ordinary label, not an OUTEQ number and not a dense slot index. Since numeric aliasing is gone, `.add(1, model)` no longer binds to an output declared as `outeq_1`; address outputs by their declared name (`.add("cp", ..)`). Models built without metadata are unaffected: numeric labels still resolve positionally there, so `.add(0, ..)` on a handwritten `ODE::new(..)` still targets the first output equation.
- `impl PartialEq<usize>` is removed from `OutputLabel` and `InputLabel` (and their reference forms). Now that `Prediction::outeq()` and `Observation::outeq()` return a label, `pred.outeq() == 0` is almost always a migration bug, and these impls were the only thing stopping the compiler from saying so — they let two in-tree call sites compile cleanly and fail at runtime instead. Migration: compare the label (`pred.outeq().as_str() == "cp"`), or, if you genuinely mean the numeric-label case, say so explicitly with `label.index() == Some(0)`.

### Fixed

- NCA silently returned no observations for models with named outputs. `Occasion::get_observations` compared `Observation::outeq_index()` against a `usize`, which is `None` for a named label, so nothing matched and NCA analysed an empty profile instead of reporting the mistake. Output selection is label-based end to end, and a label the data does not carry is now an error listing the labels that are present.
- Output and input labels are no longer destroyed during simulation. `resolve_occasion_events` used to clone the occasion and overwrite each label with its dense slot (`OutputLabel("cp")` became `OutputLabel("0")`), which allocated a `String` per event and forced `to_prediction` to parse the slot back out. Labels are resolved once per occasion into a borrowed resolved-event view that carries the slot alongside the untouched label, so the simulation path no longer clones the occasion and no longer allocates per observation.
