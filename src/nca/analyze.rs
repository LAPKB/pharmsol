...
pub(crate) fn analyze(
    ...
    let mut exposure = compute_exposure(profile, options)?;
...
fn compute_exposure(profile: &Profile, options: &NCAOptions) -> Result<ExposureParams, NCAError> {
...
fn compute_terminal(
    ...
    let terminal = lz_result.as_ref().map(|lz| {
        ...
        // MRT
        let aumc_last_val = calc::aumc_last(profile);
...
fn compute_route_specific(
    ...
    match route {
        Route::IVBolus => {
            ...
            let c0 = calc::c0_backextrap(profile, lambda_z);
            ...
            let vss = lz_result.and_then(|lz| {
                dose.map(|d| {
                    ...
                    let auc_inf = calc::auc_inf(auc_last, clast, lz.lambda_z);
                    let aumc_inf = calc::aumc_inf(aumc_last, clast, tlast, lz.lambda_z);
...
mod tests {
    ...
    fn test_empty_profile() {
        let profile = Profile::from_arrays(&[], &[], 0.0, BLQRule::Exclude);
