...
impl EM {
    ...
    fn calculate_error(&self, y1: &NalgebraVec<f64>, y2: &NalgebraVec<f64>) -> f64 {
        let n = y1.len();
        let mut err = 0.0f64;
    ...
    fn euler_maruyama_step(&self, time: f64, dt: f64, state: &mut NalgebraVec<f64>) {
        let n = state.len();
        let mut rateiv = NalgebraVec::from_vec(vec![0.0, 0.0, 0.0], NalgebraContext);
        ...
        let mut drift_term = NalgebraVec::zeros(n, NalgebraContext);
        (self.drift)(
            state,
            &self.params,
            time,
        ...
        let mut diffusion_term = NalgebraVec::zeros(n, NalgebraContext);
    ...
    pub fn solve(&mut self, t0: f64, tf: f64) -> (Vec<f64>, Vec<NalgebraVec<f64>>) {
        let mut t = t0;
